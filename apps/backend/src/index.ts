...
export class GameMatch extends DurableObject<CloudflareBindings> {
  ...
  async act(playerToken: string, action: ClientAction): Promise<MatchSnapshot> {
    const role = await this.requireRole(playerToken);
    const state = this.requireState();
  ...
  private readState(): GameState | undefined {
    ...
    const parsed = JSON.parse(row.payload) as GameState & { revision?: number };
  ...
  private expiryTtl(state: GameState): number {
    ...
    return this.openSeats() === 1 ? ACTIVE_ROOM_TTL_MS : WAITING_ROOM_TTL_MS;
  }
  ...
  private matchId(): string | undefined {
    ...
    return name?.startsWith("match:") ? name.slice("match:".length) : undefined;
  }
  ...
  private openSeats(): number {
    return Number(!this.hasSeat("dark")) + Number(!this.hasSeat("government"));
  }

  private clearDisconnectDeadline(role: PlayerRole): void {
    this.ctx.storage.sql.exec("DELETE FROM seat_disconnects WHERE role = ?", role);
  }
  ...
  private removeSeat(role: PlayerRole): GameState {
    ...
    this.ctx.storage.sql.exec("DELETE FROM socket_tickets WHERE role = ?", role);
    this.clearDisconnectDeadline(role);
  ...
  private async applyVacancies(state: GameState): Promise<GameState> {
    const nextState = this.openSeats() === 2
  ...
  private async syncDirectory(state: GameState): Promise<void> {
    ...
    const status: MatchDirectoryStatus = state.phase === "ended"
      ? "ended"
      : vacancies > 0
        ? "waiting"
        : "active";
  ...
  private sendState(
    ...
    socket.send(
      JSON.stringify({ type: "state", view: this.snapshot(state, role).view }),
    );
  }
...
const errorResponse = (
  c: {
    json: (data: { error: string }, status: 400 | 401 | 409 | 500) => Response;
  },
  error: unknown,
) => {
  const message =
